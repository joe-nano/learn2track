...
def get_spherical_harmonics_coefficients(dwi, bvals, bvecs, sh_order=8, smooth=0.006, first=False, normalize_signal_directions=True):
    ...
    normalize_signal_directions : bool
        If True, signal will have zero mean and unit variance in each direction in all nonzero voxels
    ...
    if normalize_signal_directions:
        # Normalization in each direction (zero mean, unit variance)
...
def resample_dwi(dwi, bvals, bvecs, directions=None, sh_order=8, smooth=0.006, normalize_signal_directions=True):
    ...
    """
    data_sh = get_spherical_harmonics_coefficients(dwi, bvals, bvecs, sh_order=sh_order, smooth=smooth, normalize_signal_directions=False)
