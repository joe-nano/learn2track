...
def load_tractography_dataset(subject_files, volume_manager, name="HCP", use_sh_coeffs=False, normalize_signal_directions=True):
    subjects = []
    with Timer("  Loading subject(s)", newline=True):
        for subject_file in sorted(subject_files):
            ...
            if use_sh_coeffs:
                ...
                volume = neurotools.get_spherical_harmonics_coefficients(dwi, bvals, bvecs, normalize_signal_directions=normalize_signal_directions).astype(np.float32)
            else:
                # Resample the diffusion signal to have 100 directions.
                volume = neurotools.resample_dwi(dwi, bvals, bvecs, normalize_signal_directions=normalize_signal_directions).astype(np.float32)
...
def load_tractography_dataset_from_dwi_and_tractogram(dwi, tractogram, volume_manager, use_sh_coeffs=False, bvals=None, bvecs=None, step_size=None, normalize_signal_directions=True):
    # Load signal
    signal = nib.load(dwi)
